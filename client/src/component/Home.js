...
class Home extends Component {
    constructor(props) {
      ...
      getPosts() {
        //("/posts")
        axios.get("http://localhost:8888/grocery/getAll").then(res => { //backend request use
          if (res.data.success) {
            this.setState({
      ...
      handleTextSearch=(event)=>{
        ...
        axios.get("http://localhost:8888/grocery/getAll").then(res => { 
          if (res.data.success) {
      ...
      onDelete=(id)=>{
        // console.log(id);
        axios.delete(`http://localhost:8888/grocery/deleteGroceryItem/${id}`).then((res)=>{
            ...
            <table class="table">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td><p style={{textDecoration: post.isPurchased =='true' ? 'line-through': 'none'}}>{post.itemName}</p></td>
                  {/* <td><a href={`/grocery/${post._id}`}>{post.itemName}</a></td> */}
                  <td>{post.isPurchased}</td>
                  <td>
                    <a className="btn btn-light" href={`/updatePurchaseStatus/${post._id}`}> 
                    {/* react App.js path */}
